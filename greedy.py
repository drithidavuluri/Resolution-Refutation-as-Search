...
def resolution_greedy(kb, max_iterations=100, print_steps=False):
    c=0
    for _ in range(max_iterations):
        l = len(kb)
        kb.sort(key=heuristic)  # Sort clauses based on the heuristic
...
if __name__ == "__main__":
    ...
    result = resolution_greedy(result_kb, print_steps=(m == 1))
    end_time=time.time()
